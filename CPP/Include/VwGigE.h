...
namespace VWSDK
{
	...
	class CLASS_IMPORTEXPORT VwGigE
	{
		...
		public:
			...
			RESULT GetNumCameras(OUT UINT* pNumCameras);
			RESULT GetNumCameras(IN UINT nInterfaceIndex, OUT UINT* pNumCameras);
			RESULT GetNumCameras(IN char* pInterfaceName, OUT UINT* pNumCameras);
			...
			RESULT IPAddressToIPDecimal(IN char* pIPAddress, IN size_t nSize, OUT DWORD* pIPDecimal);
			RESULT IPDecimalToIPAddress(IN DWORD dwIPDecimal, OUT char* pIPAddress, INOUT size_t* pArgSize);
